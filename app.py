...
def get_summary_text(text, num):
    try:
        ...
        user_input_with_context = f"""You will receive the lesson plan and number of images to generate below, and your task is to create a description of each image(within 150 characters) that focuses on key visual elements and essential content of the lesson. The number of Images will tell you how many description you have to create.
        ...
        ## Important Instructions ##
        Just create the description of the images and do not attach extra text along with it.
        Follow all the instruction strictly.
        Each description must not be more than 150 characters."""
...
# Function to create a prompt for image generation
def create_image_prompt(image_descriptions):
    prompt = f"""Create a realistic and educational images for each of the following descriptions:
...
def review_page():
    ...
    if send_clicked:  # Finalize the process and generate the lesson plan and images
        ...
        # Generate images based on the lesson plan
        ...
        if isinstance(image_urls, str) and image_urls.startswith("Error"):
            ...
            for i, image_url in enumerate(image_urls):
                st.image(image_url, caption=f"Lesson Illustration {i+1}")
